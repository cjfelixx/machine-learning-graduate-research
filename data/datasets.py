...
def load_gogineni(num_data):

    d_true =[0.1 , 0.1, 0.1, 0.1]
    d = [d_true[0], d_true[1], d_true[2], d_true[3]]

    # Grab new data
    new_d_true = lambda x: x.append(np.sqrt(x[-1]**2 + np.sin(np.pi*x[-4])**2) + (0.8 - 0.5 * np.exp(-(x[-2]**2)))*x[-3] )
    new_d = lambda x: x.append(np.sqrt(x[-1]**2 + np.sin(np.pi*x[-4])**2) + (0.8 - 0.5 * np.exp(-(x[-2]**2)))*x[-3]+ np.random.normal(0.0,0.1))    
    for i in range(2,num_data+2):
        new_d_true(d_true)
        new_d(d)
    u = np.hstack((np.array(d[0:num_data]).reshape(num_data,1),np.array(d[1:num_data+1]).reshape(num_data,1)))
